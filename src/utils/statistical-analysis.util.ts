...
export class StatisticalAnalysisUtil {
  ...
  static performOneWayANOVA(
    ...
    const SSW = groups.reduce((sum, group) => {
      return sum + group.values.reduce((groupSum, value) => {
   ...
   */
  static performFishersExactTest(
   ...
   */
  static performMannWhitneyUTest(
    group1: number[],
    group2: number[]
  ): HypothesisTestResult {
    const n1 = group1.length;
  ...
  private static getTCriticalValue(confidenceLevel: number, df: number): number {
    ...
    const roundedDf = df <= 10 ? 10 : df <= 20 ? 20 : df <= 30 ? 30 : df <= 50 ? 50 : df <= 100 ? 100 : 1000;
  ...
  private static assignRanks(data: number[]): number[] {
    ...
    for (let i = 0; i < indexed.length; i++) {
      ...
      const averageRank = tiedIndices.reduce((sum, idx) => sum + currentRank + idx - tiedIndices[0], 0) / tiedIndices.length;
