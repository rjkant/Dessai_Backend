...
export const biasAnalysisValidation = Joi.object({
  ...
  startDate: Joi.date().iso().required().description('Start date for analysis period'),
...
export const recommendationUpdateValidation = Joi.object({
  ...
  notes: Joi.string().max(1000).optional().description('Optional notes about the status change'),
  ...
  implementationCost: Joi.number().positive().optional().description('Actual implementation cost'),
...
export const biasAlertAcknowledgmentValidation = Joi.object({
  ...
  notes: Joi.string().max(500).optional().description('Optional notes about the acknowledgment'),

  actionTaken: Joi.string().max(200).optional().description('Brief description of action taken'),
...
export const exportValidation = Joi.object({
  ...
  includeRawData: Joi.boolean().default(false).description('Include raw data in export'),
...
export const paginationValidation = Joi.object({
  ...
  offset: Joi.number().integer().min(0).default(0).description('Number of items to skip'),
...
export const uuidParamValidation = Joi.object({
  id: Joi.string().uuid({ version: 'uuidv4' }).required().description('UUID identifier'),
});
...
export const biasAnalysisWithPaginationValidation =
  biasAnalysisValidation.concat(paginationValidation);
export const dashboardWithFiltersValidation =
  dashboardFiltersValidation.concat(paginationValidation);
